...
export class MainService {
  ...
  getCurrentPrice(stock, cb) {
    ...
    for (var i = 0; i < this.stocks.length; i++) {
      ...
      this._http.get('https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=' + stockSymbol + '&interval=1min&apikey=GT13K1EANRVG5KOS').subscribe((res) => {
        idx++;
        // to get the right symbol because of asynchronous javascript
        if (!res.json()["Error Message"]) {
          var retrievedCurrentPrice = {
            name: res.json()["Meta Data"]["2. Symbol"],
            price: res.json()["Time Series (1min)"][Object.keys(res.json()["Time Series (1min)"])[0]]["4. close"]
          };
  ...
  getPrice(stockSymbol, cb) {
    this._http.get('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=' + stockSymbol + '&apikey=GT13K1EANRVG5KOS').subscribe((res) => {
      ...
      cb(res.json()["Meta Data"]["2. Symbol"],
        currentPr,
        res.json()["Time Series (Daily)"][Object.keys(res.json()["Time Series (Daily)"])[1]]["5. adjusted close"],
        res.json()["Time Series (Daily)"][Object.keys(res.json()["Time Series (Daily)"])[0]]["6. volume"],
        res.json()["Time Series (Daily)"][Object.keys(res.json()["Time Series (Daily)"])[0]]["7. dividend amount"]);
    })
  }
};
